...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", user_reg_start)],
        states={
            AGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, user_reg_age)],
            GENDER: [MessageHandler(filters.TEXT & ~filters.COMMAND, user_reg_gender)],
            UNIVERSITY: [MessageHandler(filters.Regex("^(Boy|Girl|Other)$"), user_reg_university)],
            FACILITY: [MessageHandler(filters.TEXT & ~filters.COMMAND, user_reg_facility)],
            DISTANCES: [MessageHandler(filters.Regex("^(Горная|Пешеходная|Водная|Вело|Охота на лис)$"), user_reg_distances)],
        },
